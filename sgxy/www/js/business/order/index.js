...
business.order={
    ...
    loadList:function(status){
        ...
        var param = {"shopId":Util.common.getParameter("shopId"),"status":status,"pageSize":this.pageSize ,"pageNum":this.pageNum};
...
document.addEventListener('DOMContentLoaded', function(){loaded("order_list_all")}, false);
var myScroll;
//下拉刷新
...
function pullUpAction () {
        var url = Util.common.baseUrl +"/deal/indent/mobile/getShopOrderList.do";
        business.order.pageNum = business.order.pageNum + business.order.pageSize;
        var param = {"shopId":Util.common.getParameter("shopId"),"status":business.order.currentStatus,"pageSize":business.order.pageSize ,"pageNum":business.order.pageNum};
        Util.common.executeAjaxCallback(url, param, function(data){
            var datas ={"datas":data};
