...
customer.search = {
    ...
    //初始化加载购物车数目
    loadCartNumber: function loadCartNumber() {
    ...
    //分类下拉
    showClassify: function showClassify(obj) {
    ...
    //下拉分类菜单
    nextClassify: function nextClassify(obj) {
        var url = Util.common.baseUrl + "/weixin/cargo/classify/queryByParentId.do";
    ...
    getClassify: function getClassify(obj) {
        var conditionStr = {
            "shopId": localStorage.getItem('shopId'),
            "goodName": '',
            "pageSize": "",
    ...
    lastClassify: function lastClassify(obj) {
        var url = Util.common.baseUrl + "/weixin/cargo/classify/queryByParentId.do";
        var param = { parentId: $(obj).attr('classify') };
        //localStorage.setItem('classifyId', $(obj).attr('classify'));
        Util.common.executeAjaxCallback(url, param, function (data) {
    ...
    initGoodList: function initGoodList(conditionStr) {
    ...
    //切换排序
    changeToAsc: function changeToAsc(obj) {
        conditionStr.pageNum = 1;
        $('.empty-list').remove();
        var sortField = obj.dataset.sortField;
        var sale, price;
        if ($(obj).hasClass("ui-icon-splb-xx")) {
            // 降序->升序
    ...
    //弹出筛选框
    showSearchFilter: function showSearchFilter(obj) {
        $("#teaNavbarPopup").hide();
        $('#classifyId').css("color", "#000");
    ...
    //筛选框确定事件
    sureBtnFilter: function sureBtnFilter() {
        $('#searchFilterId').css("color", "#000");
        $("#searchFilterPopup").hide();
    ...
    getTop: function (_getTop) {
        ...
        getTop.toString = function () {
            return _getTop.toString();
        };
    ...
    //获取元素的横坐标
    getLeft: function (_getLeft) {
        function getLeft(_x2) {
            return _getLeft.apply(this, arguments);
        }

        getLeft.toString = function () {
            return _getLeft.toString();
        };
...
//# sourceMappingURL=search-result-compiled.js.map